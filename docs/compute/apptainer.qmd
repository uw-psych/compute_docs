...
``` {.python filename="python_speed_test.py"}
#!/usr/bin/env python3
...
``` bash
apptainer exec docker://python:3.10-slim python3 python_speed_test.py # <1>
```
1. The `apptainer exec` command runs a command inside an Apptainer container. The `docker://python:3.10-slim` argument tells Apptainer to use the `python:3.10-slim` image from the Docker registry. The `python3 ./python_speed_test.py` argument tells Apptainer to run the `python3` command inside the container and provide it with the argument `python_speed_test.py`.
...
apptainer exec docker://python:3.10-slim python3 python_speed_test.py
```
...
``` bash
apptainer exec docker://python:3.11-slim python3 python_speed_test.py
```
...
```bash
apptainer exec docker://python:3.10-slim python3 python_speed_test.py > py3.10.txt # <1>
apptainer exec docker://python:3.11-slim python3 python_speed_test.py > py3.11.txt # <1>
