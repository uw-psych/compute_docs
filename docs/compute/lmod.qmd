...
Lmod is run with the command `module`. This command is only available on {{< term "compute node" display="compute nodes"  >}} and will not run on the {{< term "login node" >}}, so you need to launch a [SLURM job](../compute/slurm.qmd) on {{< term "Klone" >}} to run the command.
