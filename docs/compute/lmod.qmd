...
Lmod is run with the command `module`. This command is only available on {{< glossary compute node >}}s and will not run on the {{< glossary login node >}}, so you need to launch a [SLURM job](../compute/slurm.qmd) on {{< glossary Klone >}} to run the command.
