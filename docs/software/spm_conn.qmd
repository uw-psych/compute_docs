...
Like all {{< term Lmod >}} modules, the `spm` and `conn` modules are available only on {{< term Klone >}} {{< term "compute node" display="compute nodes" >}}, and these commands will not work on the {{< term "login node" >}}.
